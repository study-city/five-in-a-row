...
class Board {
    ...
    redraw(pieces) {
        ...
        for (let i = 0; i < pieces.length; i++) {
            this.setPiece([pieces[i].xPosition, pieces[i].yPosition], pieces[i].user)
        }
    }
    ...
    isIn(clientX, clientY) {
        ...
        // 判断是否在棋盘范围内
        if (x < this.cellW / 2 || x > this.board.w - this.cellW / 2 || y < this.cellH / 2 || y > this.board.h - this.cellH / 2) {
            return false
        } else {
            return [x, y]
        }
